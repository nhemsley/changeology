...
impl TexturedCanvasView {
    fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        // Add example code file cards
        setup_example_items(&provider, window, cx);
...
/// Setup example code file cards in the provider
fn setup_example_items(
    provider: &Rc<RefCell<TexturedCanvasItemsProvider>>,
    window: &mut Window,
    cx: &mut Context<TexturedCanvasView>,
) {
    let spacing = px(300.0); // width + gap
    ...
    provider.borrow_mut().add_item(
        "canvas.rs",
        point(px(50.0) + spacing * 2.0, px(50.0)),
        window,
        cx,
        || {
            code_file_card(
    ...
    provider.borrow_mut().add_item(
        "renderer.rs",
        point(px(50.0) + spacing * 3.0, px(50.0)),
        window,
        cx,
        || {
            code_file_card(
    ...
    provider.borrow_mut().add_item(
        "item.rs",
        point(px(50.0) + spacing * 4.0, px(50.0)),
        window,
        cx,
        || {
            code_file_card(
    ...
    provider.borrow_mut().add_item(
        "tests.rs",
        point(px(50.0) + spacing * 5.0, px(50.0)),
        window,
        cx,
        || {
            code_file_card(
