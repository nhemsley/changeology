use diff::{DiffHunk, DiffHunkStatus, DiffHunkSecondaryStatus, DiffLineType};
...
fn test_diff_hunk_creation() {
    ...
    assert!(hunk.line_types.iter().all(|&lt| lt == DiffLineType::NewOnly));
...
fn test_diff_hunk_deleted() {
    ...
    assert!(hunk.line_types.iter().all(|&lt| lt == DiffLineType::OldOnly));
    assert_eq!(hunk.line_types.len(), 3);
}
